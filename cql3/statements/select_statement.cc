    ...
    if (aggregate || nonpaged_filtering) {
        return do_with(
                cql3::selection::result_set_builder(*_selection, now,
                        options, *_group_by_cell_indices),
                             ...
                             const query_options& options)
{
    ...
    if (aggregate) {
        const bool restrictions_need_filtering = _restrictions->need_filtering();
        return do_with(cql3::selection::result_set_builder(*_selection, now, options), std::make_unique<cql3::query_options>(cql3::query_options(options)),
...
void select_statement::maybe_jsonize_select_clause(database& db, schema_ptr schema) {
    ...
    if (_parameters->is_json()) {
        ...
        for (auto selectable : selectables) {
            ...
            selector_types.push_back(selectable->get_exact_type_if_known(db, schema->ks_name()));
        }
